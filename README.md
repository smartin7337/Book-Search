This assignment uses Apollo Server and a MongoDB database/Atlas to use GraphQL queries and mutations to fetch data.



![Screenshot of homepage of heroku app ](/screenshots/deployedscreenshot.png)

![Screenshot of homepage of it working ](/screenshots/example.png)

![Screenshot of homepage of login ](/screenshots/login.png)

![Link to repository](https://smartin7337.github.io/Book-Search/)

![Link to heroku](https://book-search-sv.herokuapp.com/)

